-- Drop existing function and recreate with correct name
DROP FUNCTION IF EXISTS public.validate_bank_details(text,text,text,text);

-- Create function to validate bank details
CREATE OR REPLACE FUNCTION public.validate_bank_details(
  p_bank_name TEXT,
  p_ifsc_code TEXT,
...
BEGIN
  -- Basic validation
  IF p_bank_name IS NULL OR LENGTH(p_bank_name) < 2 THEN
    RETURN FALSE;
  END IF;
  
  IF p_ifsc_code IS NULL OR LENGTH(p_ifsc_code) != 11 THEN
    RETURN FALSE;
  END IF;
  ...
  IF p_account_holder_name IS NULL OR LENGTH(p_account_holder_name) < 2 THEN
    RETURN FALSE;
  END IF;
