-- Fix the accept_product_in_order function to handle JSON parsing issues and add delivery agent logic
DROP FUNCTION IF EXISTS public.accept_product_in_order(UUID, UUID, UUID);
...
BEGIN
  ...
  -- Use a safer approach to check if product exists in order
  -- Avoid direct JSON parsing that might fail
  BEGIN
    SELECT EXISTS(
      SELECT 1 FROM orders o
      WHERE o.id = p_order_id
      AND o.items::text ILIKE '%"id":"' || p_product_id || '"%'
      AND o.items::text ILIKE '%"seller_id":"' || p_seller_id || '"%'
  ...
  -- Check if all products from all sellers are accepted
  SELECT NOT EXISTS(
    SELECT 1 
    FROM (
      SELECT DISTINCT 
        (item::json->>'id')::uuid as product_id,
        (item::json->>'seller_id')::uuid as seller_id
      FROM (
        SELECT jsonb_array_elements(order_record.items) as item
      ) items_expanded
    ) all_order_items
