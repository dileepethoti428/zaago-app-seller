-- Update notify_sellers_on_new_order to also send push notifications
  ...
  IF (TG_OP = 'INSERT' AND NEW.status IN ('pending', 'placed')) OR 
    ...
    LOOP
      ...
      PERFORM net.http_post(
        url := 'https://amhpjsmubciahslghobw.supabase.co/functions/v1/send-seller-push-notification',
        ...
        body := jsonb_build_object(
          'sellerId', seller_record.seller_id,
          'notificationData', jsonb_build_object(
            'title', 'New Order Received',
            'message', 'You have a new order from ' || COALESCE(NEW.customer_name, 'a customer'),
            'order_id', NEW.id,
            'customer_name', NEW.customer_name,
            'order_total', seller_total,
            'customer_phone', NEW.customer_phone,
      ...
      VALUES (
        ...
        'email_sent',
        jsonb_build_object(
          'action', 'seller_notification_with_push',
