-- Fixed data synchronization with nullable agent handling
...
-- 3. Make agent_id nullable in delivery_history temporarily to handle existing data
ALTER TABLE delivery_history ALTER COLUMN agent_id DROP NOT NULL;

-- 4. Add function to ensure data consistency between orders and delivery_history (with nullable agent handling)
    ...
    IF NEW.delivered = true AND (OLD.delivered = false OR OLD.delivered IS NULL) THEN
        ...
        LOOP
            ...
            IF FOUND THEN
                ...
                -- Log inventory change if security_audit_log exists
                BEGIN
...
-- 7. Create trigger for inventory sync
...
-- 8. Add order status sync function for delivery agents
...
-- 9. Create trigger for order status sync
...
-- 10. Add notification sync for delivery updates (only if notifications table exists)
...
BEGIN
    ...
    -- Send notification to each seller (only if notifications table exists)
    BEGIN
...
-- 11. Create trigger for delivery notifications
