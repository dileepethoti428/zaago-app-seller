...
-- Drop and recreate all 3 functions with discount applied

DROP FUNCTION IF EXISTS public.get_seller_stats_with_period(uuid, text);
DROP FUNCTION IF EXISTS public.get_seller_performance_trends(uuid, text);
DROP FUNCTION IF EXISTS public.get_seller_performance_summary(uuid, text);

-- 1. get_seller_stats_with_period (period param, detailed breakdown)
...
BEGIN
  ...
  SELECT
    ...
    -- total_revenue = regular + subscription (with discount)
    COALESCE((SELECT SUM(
      COALESCE((SELECT SUM(((item->>'quantity')::numeric) * ((item->>'price')::numeric * (1 - COALESCE((item->>'discount_percentage')::numeric, 0) / 100)))
    ...
    COALESCE((SELECT SUM(p.price * (1 - COALESCE(p.discount_percentage, 0) / 100) * s.quantity) FROM subscriptions s
    ...
    -- subscription_revenue (with discount)
    COALESCE((SELECT SUM(p.price * (1 - COALESCE(p.discount_percentage, 0) / 100) * s.quantity) FROM subscriptions s
     JOIN products p ON p.id = s.product_id
     WHERE p.seller_id = seller_user_id 
     AND s.is_active = true
    ...
    -- pending_subscription_revenue (with discount)
    COALESCE((SELECT SUM(
      COALESCE((SELECT SUM(((item->>'quantity')::numeric) * ((item->>'price')::numeric * (1 - COALESCE((item->>'discount_percentage')::numeric, 0) / 100)))
       FROM jsonb_array_elements(o.items) AS item
       WHERE (item->>'seller_id') = seller_user_id::text
    ...
    -- projected_daily_subscription (with discount)
    COALESCE((SELECT SUM(p.price * (1 - COALESCE(p.discount_percentage, 0) / 100) * s.quantity) FROM subscriptions s
     JOIN products p ON p.id = s.product_id
     WHERE p.seller_id = seller_user_id 
     AND s.is_active = true
...
-- 2. get_seller_performance_trends (time_range param, grouped by period)
...
-- 3. get_seller_performance_summary (time_range param, simple stats)
