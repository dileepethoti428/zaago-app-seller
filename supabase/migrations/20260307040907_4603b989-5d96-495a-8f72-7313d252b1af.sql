...
DROP FUNCTION IF EXISTS public.get_seller_subscription_handover_direct(UUID, DATE);
...
BEGIN
  ...
  -- Return individual subscription rows with customer names
  RETURN QUERY
  SELECT
    ...
    s.quantity::numeric AS total_quantity,
    c.full_name::text AS customer_name,
    s.quantity::numeric AS customer_quantity
